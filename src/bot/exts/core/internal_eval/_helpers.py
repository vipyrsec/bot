...
class EvalContext:
    ...
    def locals(self, locals_: dict[str, Any]) -> None:
        ...
        log.info(f"Updating {self._locals} with {locals_}")
    ...
    def prepare_eval(self, code: str) -> str | None:
        ...
        log.info("Parsing the AST to see if there's a trailing expression we need to capture")
        code_tree = CaptureLastExpression(code_tree).capture()

        log.info("Wrapping the AST in the AST of the wrapper coroutine")
    ...
    async def run_eval(self) -> Namespace:
        ...
        log.info("Compiling the AST to bytecode using `exec` mode")
    ...
    def format_output(self) -> str:
        ...
        log.info(f"Getting output from stdout `{id(self.stdout)}`")
        stdout_text = self.stdout.getvalue()
        if stdout_text:
            log.info("Appending output captured from stdout/print")
            output.append(stdout_text)

        if self._value_last_expression is not None:
            log.info("Appending the output of a captured trialing expression")
            output.append(f"[Captured] {self._value_last_expression!r}")

        if self.exc_info:
            log.info("Appending exception information")
            output.append(format_internal_eval_exception(self.exc_info, self.code))

        log.info(f"Generated output: {output!r}")
...
class CaptureLastExpression(ast.NodeTransformer):
    ...
    def visit_Expr(self, node: ast.Expr) -> ast.Expr | ast.Assign:  # noqa: N802
        ...
        log.info("Found a trailing last expression in the evaluation code")

        log.info("Creating assignment statement with trailing expression as the right-hand side")
