...
class InternalEval(commands.Cog):
    ...
    async def _eval(self, ctx: commands.Context, code: str) -> None:
        ...
        log.info("Preparing the evaluation by parsing the AST of the code")
        error = eval_context.prepare_eval(code)

        if error:
            log.info("The code can't be evaluated due to an error")
            await ctx.send(f"```py\n{error}\n```")
            return

        log.info("Evaluate the AST we've generated for the evaluation")
        new_locals = await eval_context.run_eval()

        log.info("Updating locals with those set during evaluation")
        self.locals.update(new_locals)

        log.info("Sending the formatted output back to the context")
